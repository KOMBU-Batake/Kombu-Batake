...
public:
	...
	int identifyWall(LiDAR_degree direction) {
		// 73個全部取得
		vector<float> examinee(73);
		getRangeImage(examinee, direction);
		float max = *max_element(examinee.begin(), examinee.end()); // 最大値
		float min = *min_element(examinee.begin(), examinee.end()); // 最小値
		if (max > 18){
			if (min > 18) {
				return 0;
			}
...
private:
	...
	// 中心±36度の範囲の点を取得
	void getRangeImage(vector<float>& examinee, const LiDAR_degree& direction) {
	...
	void printVectorExcel2D(vector<XZcoordinate>& vec, int j);
