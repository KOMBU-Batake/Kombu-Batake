...
void MapperS::markAroundStatus(const aroundStatus& status, const double& angle, const bool& isFirst)
{
	...
	if ((!((angle >= 0 && angle < 5) || (angle > 355 && angle <= 360))) || isFirst) map_S[currentTile_L.z - 1][currentTile_L.x] = around4[0]; // 上
	if ((abs(angle - 270) >= 5) || isFirst) map_S[currentTile_L.z][currentTile_L.x + 1] = around4[3]; // 右
	if ((abs(angle - 180) >= 5) || isFirst) map_S[currentTile_L.z + 1][currentTile_L.x] = around4[2]; // 下
	if ((abs(angle - 90 ) >= 5) || isFirst) map_S[currentTile_L.z][currentTile_L.x - 1] = around4[1]; // 左
}
